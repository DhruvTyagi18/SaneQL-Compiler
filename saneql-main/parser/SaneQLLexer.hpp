...
#undef KEYWORD
   ...
   std::experimental::string_view::iterator current;
   /// A forced token (if any)
   Token forcedToken;

   public:
   /// Constructor
   explicit SaneQLLexer(std::experimental::string_view input, Token forcedToken = Token::Eof);

   /// Access the full text
   std::experimental::string_view getFullText() const { return input; }
   /// Get the current position
   std::experimental::string_view::iterator savePosition() const { return current; }
   /// Go back to a previously saved position
   void restorePosition(std::experimental::string_view::iterator p) { current = p; }
   ...
   std::experimental::string_view getRangeFromBounds(unsigned beginColumn, unsigned endColumn) const { return std::experimental::string_view{input.begin() + beginColumn - 1, endColumn - beginColumn}; }
   /// Check if a symbol is a keyword
   static bool isKeyword(std::experimental::string_view symbol);
